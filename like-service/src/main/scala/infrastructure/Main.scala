...
object Main extends App {
  ...
  props.put(StreamsConfig.APPLICATION_ID_CONFIG, config.getString("like-service.application-id"))
  props.put(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, config.getString("like-service.kafka-bootstrap-servers"))
  props.put(StreamsConfig.ZOOKEEPER_CONNECT_CONFIG, config.getString("like-service.zookeeper-connect"))
  props.put(AbstractKafkaAvroSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, config.getString("like-service.schema-registry-url"))
  ...
  val tweetsTopic = config.getString("like-service.tweets-topic")
  val usersTopic = config.getString("like-service.users-topic")
  val likesTopic = config.getString("like-service.likes-topic")
