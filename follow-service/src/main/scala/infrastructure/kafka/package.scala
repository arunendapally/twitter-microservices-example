...
package object kafka {
  ...
  implicit def functionToValueJoiner[V1, V2, R](f: (V1, V2) => R): ValueJoiner[V1, V2, R] = 
    new ValueJoiner[V1, V2, R] {
  ...
  implicit def functionToProcessorSupplier[K, V](f: => Processor[K, V]): ProcessorSupplier[K, V] = 
    new ProcessorSupplier[K, V] {
      override def get(): Processor[K, V] = f
    }
}
